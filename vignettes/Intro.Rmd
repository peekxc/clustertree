...
```{r}
S <- sample_mix(150, f_list = list(mix1, mix2, mix3))
X_n <- S[, 1:2]
I <- S[, 3]
...
m <- diag(nrow(X_n))
m_con <- mapply(function(x, y) { ifelse(I[x] == I[y], 1, -1) }, row(m), col(m), SIMPLIFY = T)
constraints <- matrix(m_con, nrow = nrow(X_n), byrow = F)

ARI_scores <- sapply(C_n$hc$height, function(h) ARI(cut_with_noise(C_n$hc, h+.Machine$double.eps), I))

opt_ext <- dbscan::extractFOSC(C_n$hc, constraints = constraints, alpha = 0)
ARI(, I)
plot(X_n, col = cut_with_noise(C_n$hc, h = C_n$hc$height[which.max(ARI_scores)]) + 1)
plot(X_n, col = opt_ext$cluster+1)

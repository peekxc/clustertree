...
r_k <- apply(dbscan:::kNNdist(X_n, k = 6), 1, max)
sum(clustertree:::primsRSL(dist(X_n), r_k = r_k, n = nrow(X_n), alpha = sqrt(2), type = 0)[, 3])

what <- clustertree::clustertree(X_n, k = 5, alpha = sqrt(2), estimator = "RSL")
...
cppFunction('bool double_max() {
  return NA_REAL < std::numeric_limits<double>::infinity();
}')

...
ANNkd_ptr rkd_tree_dt(				// recursive construction of kd-tree
    ...
    std::unordered_map<ANNkd_node*, node_bnb>& node_info) // node info
{
  // Compute and store the maximum radius around the bounding box
  node_bnb c_node_info = node_bnb();
  c_node_info.id = g_id++;
  // c_node_info.lo = annCopyPt(dim, bnd_box.lo);
  // c_node_info.hi = annCopyPt(dim, bnd_box.hi);
  ...
  c_node_info.max_radius = annDist(dim, bnd_box.lo, bnd_box.hi)/2;
  ...
  if (n <= bsp) {						// n small, make a leaf node
    if (n == 0)						// empty leaf node
      ...
      node_info[new_leaf] = c_node_info;
      return new_leaf;
    }
  }

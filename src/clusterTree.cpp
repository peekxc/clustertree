...
// Robust Single Linkage step
// std::max(r[d_i] / alpha, std::max(r_k[c_i], r_k[t_i]));
...
*/
NumericMatrix primsMST(const NumericVector r, const NumericVector r_k, const int n, const double alpha){
...
/* - mstToHclust
 * Given a minimum spanning tree, create a valid hclust object
...
List clusterTree(const NumericVector x, const NumericVector r_k, const int k, const double alpha = 1.414213562373095) {
  ...
  NumericVector r = Rcpp::clone(x);

  // Run the MST with set parameters
  NumericMatrix mst = primsMST(r, r_k, n, alpha);

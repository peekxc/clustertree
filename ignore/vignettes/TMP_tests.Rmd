...
sum(clustertree:::primsRSL(dist(X_n), r_k = r_k, n = nrow(X_n), alpha = sqrt(2), type = 0)[, 3])

what <- clustertree::clustertree(X_n, k = 5, alpha = sqrt(2), estimator = "mutual knn")

## Supply Variables/parameters
{ n <- nrow(X_n); dist_x <- dist(X_n, method = "euclidean") }


sum(hclust(dist(as.matrix(iris[1:5, 1:4])), method = "single")$height)

clustertree:::knn1(as.matrix(iris[, 1:4]))

cppFunction('bool double_max() {
  double x = NA_REAL; 
  return(R_IsNA(x));
}', plugin="cpp11")
...
```{r}
what <- clustertree::clustertree(X_n, k = 5, estimator = "mutual knn")
```
